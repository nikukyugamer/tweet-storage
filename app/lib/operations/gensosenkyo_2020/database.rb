    ...
    class Database
      class << self
        ...
        def write_by_next_tweet_id_number_search(search_query)
          ...
          ::Database::SaveTweet.by_search_word_tweet(search_query, tweets) if tweets.present?
        end
        ...
        def write_by_list(list_identify, options = { tweet_mode: 'extended', count: 200, since_id: 1, max_id: 9_000_000_000_000_000_000 })
          ...
          ::Database::SaveTweet.by_list_tweet(target_list_from_api, tweets) if tweets.present?
        end
        ...
        def write_by_specific_user_tweet(user_identify, options = { tweet_mode: 'extended', count: 200, since_id: 1, max_id: 9_000_000_000_000_000_000 })
          ...
          ::Database::SaveTweet.by_specific_user_tweet(tweets) if tweets.present?
        end
        ...
        def write_specific_user_record(user_identify)
          ...
          ::Database::SaveUser.create(specific_user) unless specific_user.nil?
        end

        # 一つだけであることに注意する
        # FIXME: 取得に失敗した場合には例外が返るのでそのための処理が必要
        def write_tweet_by_specific_tweet_id_number(id_number)
          ...
          ::Database::SaveTweet.by_specific_id_tweets(tweet) unless tweet.nil?
        end
        ...
        def write_tweets_by_specific_tweet_id_numbers(*id_numbers)
          ...
          ::Database::SaveTweet.by_specific_id_tweets(tweets) if tweets.present?
        end

        # 特定のリストのレコードを追加する
        def write_specific_list_record(id_number)
          specific_list = ::TwitterApi::CollectList.specific_list(id_number)
          ::Database::SaveList.create(specific_list) unless specific_list.nil?
        end
      end
    end
  end
